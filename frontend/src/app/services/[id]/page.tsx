  ...
  return (
    <div className="min-h-screen bg-slate-50 pt-32 pb-24 overflow-hidden gpu">
      ...
      <div className="container mx-auto px-4 lg:px-8 max-w-7xl">
        ...
        {/* Hero Section */}
        <div className="grid lg:grid-cols-2 gap-8 lg:gap-24 mb-16 md:mb-32 items-center">
          <div className="js-reveal space-y-6">
            ...
            <div className="flex flex-wrap gap-4 pt-4">
              <Link href="/appointment" className="w-full sm:w-auto">
                <button className={`w-full sm:w-auto px-8 py-4 text-white rounded-full font-black text-base transition-all hover:scale-105 active:scale-95 shadow-xl flex items-center justify-center gap-3 ${service.category === 'Taxation Division' ? 'bg-emerald-600 shadow-emerald-200' : 'bg-blue-600 shadow-blue-200'}`}>
              ...
              <div className="space-y-6">
                {service.features.map((feature, idx) => (
                  <div key={idx} className="item-reveal flex items-start gap-5 p-6 bg-slate-50/50 rounded-3xl border border-transparent hover:border-blue-100 hover:bg-white transition-all group">
                    <CheckCircle className="w-5 h-5 text-emerald-500 mt-1 flex-shrink-0 group-hover:scale-110 transition-transform" />
                    <p className="text-base font-bold text-slate-700 leading-tight tracking-tight uppercase">
                      {feature}
                    </p>
                  </div>
                ))}
              ...
              <div className="space-y-6">
                {service.benefits.map((benefit, idx) => (
                  <div key={idx} className="item-reveal flex items-start gap-5 p-6 bg-white/5 rounded-3xl border border-transparent hover:border-white/10 hover:bg-white/10 transition-all group">
                    ...
                    <p className="text-base font-bold text-slate-300 leading-tight tracking-tight uppercase">
                      {benefit}
                    </p>
                  </div>
                ))}
        ...
        {/* Global Support Card */}
        <div className="js-reveal mt-16 md:mt-32 bg-gradient-to-br from-slate-50 to-white rounded-[2rem] md:rounded-[3.5rem] p-8 md:p-20 border border-slate-100 text-center shadow-2xl shadow-slate-200 relative overflow-hidden">
           <div className="relative z-10 max-w-3xl mx-auto">
              <Users className="w-12 h-12 md:w-16 md:h-16 text-blue-600 mx-auto mb-6 md:mb-10" />
              ...
              <div className="flex flex-col sm:flex-row gap-4 justify-center">
                 <button className="w-full sm:w-auto px-8 py-4 bg-slate-900 text-white rounded-full font-black text-base hover:bg-slate-800 transition-all flex items-center justify-center gap-3">
                    <Phone className="w-5 h-5" /> Call Advisor
                 </button>
                 <button className="w-full sm:w-auto px-8 py-4 border-2 border-slate-900 text-slate-900 rounded-full font-black text-base hover:bg-slate-50 transition-all flex items-center justify-center gap-3">
